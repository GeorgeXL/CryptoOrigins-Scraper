...
export function TagManager() {
  ...
  const moveTagMutation = useMutation({
    mutationFn: async ({ tagId, category, subcategoryKey }: { tagId: string; category: string; subcategoryKey: string }) => {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || `Failed to move tag: ${response.statusText}`);
      }
  ...
  const addSubcategoryMutation = useMutation({
    mutationFn: async ({ category, parentPath, name }: { category: string; parentPath: string[]; name: string }) => {
      const response = await fetch('/api/tags/subcategory', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || `Failed to create subcategory: ${response.statusText}`);
      }
  ...
  const renameSubcategoryMutation = useMutation({
    mutationFn: async ({ category, subcategoryKey, newName }: { category: string; subcategoryKey: string; newName: string }) => {
      const response = await fetch('/api/tags/subcategory/rename', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || `Failed to rename subcategory: ${response.statusText}`);
      }
  ...
  const deleteSubcategoryMutation = useMutation({
    mutationFn: async ({ category, subcategoryKey, action }: { category: string; subcategoryKey: string; action: 'delete' | 'move_to_parent' }) => {
      const response = await fetch('/api/tags/subcategory/delete', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || `Failed to delete subcategory: ${response.statusText}`);
      }
