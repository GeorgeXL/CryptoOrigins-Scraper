...
// Test the battle process for 2 entries
async function testBattle2Entries() {
  console.log('⚔️ Testing Battle Process for 2 Entries...\n');
  ...
  try {
    ...
    // Find AI Arena entries (excluding orphans)
    const arenaAnalyses = allAnalyses.filter(analysis => {
      ...
      // Exclude orphans
      if (analysis.isOrphan === true) return false;
      
      return (!isPerplexityVerified && !isOpenAIVerified && (hasPerplexityVerdict || hasOpenAIVerdict)) ||
             (isOneVerified && isGeminiRejected);
    });

    console.log(`⚔️ Found ${arenaAnalyses.length} AI Arena entries (excluding orphans)\n`);
    ...
    // Use first 2 entries
    const testAnalyses = arenaAnalyses.slice(0, 2);

    for (let i = 0; i < testAnalyses.length; i++) {
      results.total++;
      const testAnalysis = testAnalyses[i];
      console.log(`\n${'='.repeat(80)}`);
      console.log(`🧪 BATTLE TEST ${i + 1}/2: ${testAnalysis.date}`);
...
Return ONLY the shortened summary (100-110 chars), nothing else.`;
    ...
    console.log('='.repeat(80));
    console.log('\n✅ Battle test completed for 2 entries!');
...
testBattle2Entries().catch(console.error);
